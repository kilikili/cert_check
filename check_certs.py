...
def get_cert_info(host, port=443, timeout=5, verify=False):
    """取得憑證資訊並產生安全建議"""
    advice = []
    expiry_utc = None
    tls_version = "N/A"
...
def check_host(host, port, now_utc, notify_days):
    ...
    try:
        ...
        expiry_local = expiry_utc.astimezone(LOCAL_TZ) if expiry_utc else "N/A"
        ...
        return [host, port, expiry_local if expiry_local != "N/A" else "", days_left, status, tls_version, sig_algo, key_size, advice]
    except Exception as e:
        return [host, port, "", "", f"ERROR: {e}", "N/A", "N/A", "N/A", f"檢查失敗：{e}"]

def main():
    parser = argparse.ArgumentParser(description="SSL 憑證到期監控工具 (多執行緒 + 資安建議)")
    ...
    with open(CSV_FILE, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["Host", "Port", "Expiry(Local)", "Days Left", "Status", "TLS Version", "Signature", "Key Size", "Security Advice"])
